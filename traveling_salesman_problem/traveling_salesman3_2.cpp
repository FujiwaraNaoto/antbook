...
int main(){
    ...
    int ans=INF;
    vector<int> ans_to(n,-1);
    int ans_start=-1;

    for(int start=0;start<n;start++){

        vector<vector<int>> dp((1<<n),vector<int>(n,INF));
        dp[(1<<n)-1][start]=0;//startが始まりかつゴール
        
        vector<int> to(n,-1);//to[i]でiの行先がto[i]
        to[start]=start;
    ...
    for(int u=0;u<n;u++){
        ...
        cout<<"from "<<mp_rev[u]<<" to "<<mp_rev[ans_to[u]]<<endl;
        
        //cout<<"ans_to["<<mp_rev[u]<<"]="<<mp_rev[ans_to[u]]<<endl;
    }
